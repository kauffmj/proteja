...
public class ConstructMatrix
{
    public static void main(String[] args)
    {
        ...
        /**
         * Read in settings file and update values, read in first array so
         * information about the size can be used.
         */
        int numberOfTests = 0;
        ...
        try
        {
            ...
            //matrixFileName += (set.getCoverageType() + "-coverage.dat");
            batchFactor = set.getBatchFactor();
            current = arrayRetrieveFromXML(arrayFile + "1.out");
        } catch (IOException e)
        {
            System.out.println("There was a problem reading the arrays");
            System.out.println("Upper");
            System.exit(1);
        }

        /**
         * if batchFactor is 1 then different naming conventions are used
         * then if batchFacto is larger than 1. This section reads in the arrays
         * and fills the matrix column at a time.  THIS SECTION IS OUTDATED
         */
        /**
        if (batchFactor == 1) {
         */
            coverageMatrix = new int[current.length + 1][numberOfTests + 1];
            for (int i = 1; i <= numberOfTests; i++) {
                try {
                    current = arrayRetrieveFromXML(arrayFile + i + ".out");
                } catch (IOException e) {
            ...
            coverageMatrix = new int[current.length + 1][numberOfFiles + 1];
            int currentColumn = 0;
...
*/
        ...
        try
        {
            writeMatrix(matrixToString(coverageMatrix), matrixFileName);
        } catch (IOException e)
        {
            System.out.println("There was a problem writing the coverage " +
                    "to disk!");
            e.printStackTrace();
            System.exit(1);
        }
     ...
     */
    public static void writeMatrix(String str, String file) throws IOException
    {
     ...
     */
    public static int[] arrayRetrieveFromXML(String name) throws IOException
    {
        XStream xstream = new XStream();
        FileReader fileIn = new FileReader(name);
        BufferedReader read = new BufferedReader(fileIn);
    ...
    {
        StringBuilder sb = new StringBuilder(matrix.length*matrix[0].length*3);
