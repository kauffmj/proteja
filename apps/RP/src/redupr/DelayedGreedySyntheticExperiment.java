...
public class DelayedGreedySyntheticExperiment
{
	public static void main(String[] args)throws IOException
	{
		...
		File resultsFile = new File("data/diatoms/reduce/synthetic_test_results/DelayedGreedySyntheticTestResults.dat");
					...
					while (coverageAmountIterator.hasNext())
					{
						...
						while (metricsIterator.hasNext())
						{
							...
							for(int replicationFactor=1;replicationFactor<=10;replicationFactor++)
							{
								...
								ReduceUsingDelayedGreedy dg = new ReduceUsingDelayedGreedy(SetCover.constructSetCoverFromBinary(DIRTODATA+"matrix-"+config,DIRTODATA+"time-"+config));
								...
								String coveringSet = dg.getCoveringTestSetString("Q");
								
								dg.setSetCover(SetCover.constructSetCoverFromBinary(DIRTODATA+"matrix-"+config,DIRTODATA+"time-"+config));
								System.gc();
								start = System.currentTimeMillis();
								...
								expCounter++;
								System.out.println("Delayed Greedy Experiment: "+expCounter+"/1620 "+config+" "+metric+" "+totalTime+"ms" );
								
								String prioritizeSet = dg.getPrioritizedSetString("Q");
								
								
								try 
								{
									resultsWriter.write(config+"\tDG\t"+metric+"\t"+reduceTime+"\t"+coveringSet+"\t"+
