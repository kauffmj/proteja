/*---------------------------------------------------------------------
 *  File: $Id: TestReduceUsingGreedy.java,v 1.26 2007/07/02 13:36:17 smitha Exp $   
...
/**
 *  Test suite for the ReduceUsingGreedy class.
 *
 *  @author Adam M. Smith 7/01/2007
 */

public class MyGreedySyntheticExperiment
{
	public static void main(String[] args)throws IOException
	{
		...
		File resultsFile = new File("data/diatoms/reduce/synthetic_test_results/MyGreedySyntheticTestResults.dat");
					...
					while (coverageAmountIterator.hasNext())
					{
						...
						while (metricsIterator.hasNext())
						{
							...
							for(int replicationFactor=1;replicationFactor<=10;replicationFactor++)
							{
								...
								ReduceUsingGreedy g = new ReduceUsingGreedy(SetCover.constructSetCoverFromBinary(DIRTODATA+"matrix-"+config,DIRTODATA+"time-"+config));
							
								System.gc();
								start = System.currentTimeMillis();
								g.reduceUsingGreedy(metric);
								stop = System.currentTimeMillis();
								
								reduceTime = stop-start;
								
								String coveringSet = g.getCoveringTestSetString("Q");
								
								g.setSetCover(SetCover.constructSetCoverFromBinary(DIRTODATA+"matrix-"+config,DIRTODATA+"time-"+config));
								System.gc();
								start = System.currentTimeMillis();
								g.prioritizeUsingGreedy(metric);
								stop= System.currentTimeMillis();
								
								prioritizeTime = stop-start;
								
								expCounter++;
								System.out.println("My Greedy Experiment: "+expCounter+"/1620 "+config+" "+metric+" "+totalTime+"ms" );
								
								String prioritizeSet = g.getPrioritizedSetString("Q");
								
								try 
								{
									resultsWriter.write(config+"\tmyGreedy\t"+metric+"\t"+reduceTime
																+"\t"+coveringSet+"\t"+
